...
private:
    vector<vector<GLfloat>> v;  //放頂點(x,y,z)
    vector<vector<GLint>> f;    //存三頂點index
    vector<vector<GLfloat>> vt; //放texture點(a,b)
    vector<vector<GLint>> fvt;  //存三頂點trexture index
    vector<vector<GLfloat>> vn; //放normal頂點(x,y,z)
    vector<vector<GLint>> fvn;  //存三頂點normal index
    ...
    void drawModeLine() {
        ...
        for (int i = 0; i < f.size(); i++) {
            ...
            if ((f[i]).size() != 3) {
                ...
                GLint firstVertexIndex = (f[i])[0];//頂點index
